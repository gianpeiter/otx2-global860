...
function creatureSayCallback(cid, type, msg)
	...
	local teletransporte = { x = 32202, y = 31812, z = 8 }

	if
		msgcontains(msg, "seventh") and getPlayerLevel(cid) < 60
		or msgcontains(msg, "last") and getPlayerLevel(cid) < 60
	then
		selfSay(
			"You are not experienced enough to master the challenges ahead or to receive knowledge about the seventh seal. Go and learn more before asking me again.",
			cid
		)
	elseif
		msgcontains(msg, "seventh seal")
		or msgcontains(msg, "seventh") and getPlayerLevel(cid) > 59
		or msgcontains(msg, "last") and getPlayerLevel(cid) > 59
	then
		selfSay(
	...
	elseif msgcontains(msg, "kiss") and setimo_selo == -1 and talk_state == 8 then
		...
		talk_state = 1
	elseif
		msgcontains(msg, "kiss")
		and getPlayerStorageValue(cid, 32993) == 1
		and getPlayerStorageValue(cid, 32994) == 1
		and getPlayerStorageValue(cid, 32995) == 1
		and getPlayerStorageValue(cid, 32996) == 1
		and getPlayerStorageValue(cid, 32997) == 1
		and getPlayerStorageValue(cid, 32998) == 1
		and getPlayerStorageValue(cid, 32992) == -1
	then
	...
	elseif msgcontains(msg, "yes") and talk_state == 5 and terceiro_selo == 1 then
		selfSay(
			"Ah, I can sense the power of the seal of demonrage burning in your heart. Have you passed any other seal yet?",
			cid
		)
		talk_state = 6
	elseif msgcontains(msg, "yes") and talk_state == 5 and terceiro_selo == -1 then
	...
	elseif msgcontains(msg, "yes") and talk_state == 7 and sexto_selo == 1 then
		selfSay(
			"I see! You have mastered the seal of logic. You have made the sacrifice, you have seen the unseen, you possess fortitude, you have filled yourself with power and found your path. You may ask me for my {kiss} now.",
			cid
		)
		talk_state = 8
	elseif msgcontains(msg, "yes") and talk_state == 7 and sexto_selo == -1 then
	...
	elseif msgcontains(msg, "spectral dress") then
		selfSay(
			"Your wish for a spectral dress is silly. Allthough I will grant you the permission to take one. My maidens left one in a box in a room, directly south of here.",
			cid
		)
	...
	elseif
		(msgcontains(msg, "holy") or msgcontains(msg, "Holy"))
		and (msgcontains(msg, "orchid") or msgcontains(msg, "Orchid"))
		and getPlayerStorageValue(cid, 6870) == 6
	then
