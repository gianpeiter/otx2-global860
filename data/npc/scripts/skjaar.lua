...
function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "yes") and state == 2 then
		if doPlayerRemoveMoney(cid, 1000) then
			npcHandler:say(
				"All right then. Here comes the first question. What was the name of Dago's favourite pet?",
				cid
			)
			talkState[cid] = 3
		else
	...
	elseif msgcontains(msg, "7") and state == 4 then
		npcHandler:say(
			"Also true. But can you also tell me the colour of the deamons in which master specialized?",
			cid
		)
		talkState[cid] = 5
