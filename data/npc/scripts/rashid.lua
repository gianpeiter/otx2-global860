	...
	elseif Topic[cid] == 6 then -- Finish mission 1 - The interrogation
		if msgcontains(msg, "yes") and getPlayerStorageValue(cid, 100008) >= 2 then
			if doPlayerRemoveItem(cid, 7503, 1) then
				npcHandler:say(
					"Great. Just place it over there - yes, thanks, that's it. Say {mission} to continue.",
					cid
				)
	...
	elseif Topic[cid] == 9 then -- Finish mission 1 - The interrogation
		if msgcontains(msg, "yes") then
			if doPlayerRemoveItem(cid, 8112, 1) then
				npcHandler:say(
					"Mmmhh, the lovely odeur of scarab cheese! I really can't understand why most people can't stand it. Thanks, well done! Say {mission} to continue.",
					cid
				)
	...
	elseif Topic[cid] == 12 then -- Finish mission 1 - The interrogation
		if msgcontains(msg, "yes") then
			if doPlayerRemoveItem(cid, 7582, 1) then
				npcHandler:say(
					"I'm surprised that you managed to bring this vase without a single crack. That was what I needed to know, thank you. Thanks, well done! Say {mission} to continue.",
					cid
				)
	...
	elseif Topic[cid] == 15 then -- Finish mission 1 - The interrogation
		if msgcontains(msg, "yes") and getPlayerStorageValue(cid, 100010) == 1 then
			if doPlayerRemoveItem(cid, 8209, 1) then
				npcHandler:say(
					"Ha! You are clever indeed, well done! I'll take this from you. I think we two might get into business after all. Say {mission} to continue.",
					cid
				)
...
function onTradeRequest(cid)
	if getPlayerStorageValue(cid, 100007) < 13 then
