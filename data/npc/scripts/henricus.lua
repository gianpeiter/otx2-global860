...
function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "ruthless seven") then
		npcHandler:say(
			"They're one of the mightiest weapons that our enemies have at disposal. They're the leaders of some of the most powerful forces of demonkind.",
			cid
		)
		Topic[cid] = 0
	elseif
		(msgcontains(msg, "inquisition") or msgcontains(msg, "join") or msgcontains(msg, "mission"))
		and getPlayerStorageValue(cid, storage) == -1
	then
	...
	elseif msgcontains(msg, "mission") then
		...
		elseif v == 3 then -- Start Mission 2 - The Eclipse
			npcHandler:say(
				"Listen, we have information about a heretic coven that hides in a mountain called the Big Old One. The witches reach this cursed place on flying brooms and think theyare safe there. ...",
				cid
			)
			npcHandler:say(
		...
		elseif v == 15 then
			npcHandler:say(
				"Your mission is simple,You must enter in the retreat and destruct the Shadow Nexus. Then report to me about your {mission}.",
				cid
			)
	...
	elseif Topic[cid] == 2 then -- Finish mission 1 - The interrogation
		if msgcontains(msg, "yes") then
			if getPlayerStorageValue(cid, 13890) == 4 then
				...
				npcHandler:say(
					"I don't think so. This failure will be noted down. However, continue your investigations and return to me after you're done.",
					cid
				)
			end
		else
			npcHandler:say(
				"This lax behaviour is very suspicious. You better don't return here until you've finished your mission.",
				cid
			)
		end
		Topic[cid] = 0
	elseif Topic[cid] == 3 then -- Finish mission 2 - The Eclipse
		if msgcontains(msg, "yes") then
			if getPlayerStorageValue(cid, 99220) == 1 and doPlayerRemoveItem(cid, 8702, 1) then
				npcHandler:say(
					"Fine, fine. You have proven that you can work efficiently. Still, only further missions will show if you're are truly capable. Ask me for another {mission} if you're ready.",
					cid
				)
				...
				npcHandler:say(
					"I don't think so. This failure will be noted down. However, continue your mission and return to me when you're done.",
					cid
				)
			end
		else
	...
	elseif Topic[cid] == 6 then -- Finish mission 4 - Libert Bay
		if msgcontains(msg, "yes") then
			if getPlayerStorageValue(cid, 49312) == 2 then
				npcHandler:say(
					"Well, this was an easy task, but your next {mission} will be much more challenging.",
					cid
				)
	...
	elseif Topic[cid] == 8 then -- Finish mission 6 - Ungreez
		if msgcontains(msg, "yes") then
			if getPlayerStorageValue(cid, 57684) == 2 then
				...
				npcHandler:say(
					"Our other operatives were not that lucky, though. But you will learn more about that in your next {mission}. Until then, you may talk to me about your {outfit}.",
					cid,
					3000
				)
	...
	elseif Topic[cid] == 9 then -- Finish mission 7 - Destroy Shadow Nexus
		if msgcontains(msg, "yes") then
			if getPlayerStorageValue(cid, 100080) == 3 then
				...
				npcHandler:say(
					"My informants tell me otherwise. Don't forget that the inquisition has its eyes everywhere! Finish your {mission} or face the wrath of the inquisition.",
					cid
				)
			end
		else
	...
	elseif msgcontains(msg, "church") then
		npcHandler:say(
			"The churches of the gods united to fight heresy and dark magic. They are the shield of the true believers, while the inquisition is the sword that fights all enemies of virtuousness.",
			cid
		)
		Topic[cid] = 0
	elseif msgcontains(msg, "gods") then
		npcHandler:say(
			"We owe to the gods of good our creation and continuing existence. If it weren't for them, we would surely fall prey to the minions of the vile and dark gods.",
			cid
		)
		Topic[cid] = 0
	...
	elseif msgcontains(msg, "crunor") then
		npcHandler:say(
			"The church of Crunor works closely together with the druid guild. This makes a cooperation sometimes difficult.",
			cid
		)
		Topic[cid] = 0
	elseif msgcontains(msg, "zathroth") then
		npcHandler:say(
			"We can see his evil influence almost everywhere. Keep your eyes open or the dark one will lead you on the wrong way and destroy you.",
			cid
		)
		Topic[cid] = 0
	...
	elseif msgcontains(msg, "carlin") then
		npcHandler:say(
			"Carlin is a city of sin and heresy. After the reunion of Carlin with the kingdom, the inquisition will have much work to purify the city and its inhabitants.",
			cid
		)
		Topic[cid] = 0
	elseif msgcontains(msg, "demon") then
		npcHandler:say(
			"Demons exist in many different shapes and levels of power. In general, they are servants of the dark gods and command great powers of destruction.",
			cid
		)
		Topic[cid] = 0
	elseif msgcontains(msg, "darashia") then
		npcHandler:say(
			"Darashia is a godless town full of mislead fools. One day, it will surely share the fate of its sister town Drefia.",
			cid
		)
		Topic[cid] = 0
	elseif msgcontains(msg, "drefia") then
		npcHandler:say(
			"Drefia used to be a city of sin and heresy, just like Carlin nowadays. One day, the gods decided to destroy this town and to erase all evil there.",
			cid
		)
		Topic[cid] = 0
	elseif msgcontains(msg, "venore") then
		npcHandler:say(
			"Venore is somewhat difficult to handle. The merchants have a close eye on our activities in their city and our authority is limited there. However, we will use all of our influence to prevent a second Carlin.",
			cid
		)
		Topic[cid] = 0
	...
	elseif msgcontains(msg, "elves") or msgcontains(msg, "ab'dendriel") then
		npcHandler:say(
			"Those elves are hardly any more civilised than orcs. They can become a threat to mankind at any time.",
			cid
		)
		Topic[cid] = 0
	elseif msgcontains(msg, "dwarf") or msgcontains(msg, "kazordoon") then
		npcHandler:say(
			"The dwarfs are allied with Thais but follow their own obscure religion. Although dwarfs keep mostly to themselves, we have to observe this alliance closely.",
			cid
		)
		Topic[cid] = 0
	elseif msgcontains(msg, "druid") then
		npcHandler:say(
			"The druids here still follow the old rules. Sadly, the druids of Carlin have left the right path in the last years.",
			cid
		)
		Topic[cid] = 0
	elseif msgcontains(msg, "sorcerer") then
		npcHandler:say(
			"Those who wield great power have to resist great temptations. We have the burden to eliminate all those who give in to the temptations.",
			cid
		)
		Topic[cid] = 0
	elseif msgcontains(msg, "knight") then
		npcHandler:say(
			"Nowadays, most knights seem to have forgotten the noble cause to which all knights were bound in the past. Only a few have remained pious, serve the gods and follow their teachings.",
			cid
		)
		Topic[cid] = 0
	elseif msgcontains(msg, "paladin") then
		npcHandler:say(
			"It's a shame that only a few paladins still use their abilities to further the cause of the gods of good. Too many paladins have become selfish and greedy.",
			cid
		)
		Topic[cid] = 0
	end
