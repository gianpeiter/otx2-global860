...
function messageDefaultCallback(cid, type, msg)
	...
	if
		msgcontains(msg, "asshole")
		or msgcontains(msg, "whore")
		or msgcontains(msg, "fuck")
		or msgcontains(msg, "bitch")
	then
		selfSay("Take this!")
	...
	if talk[cid] == 3 then
		if msg:lower() == "trouble will arise in the near future" then
			npcHandler:say(
				"I think the gods and the government do their best to keep away harm from the citizens.",
				cid
			)
	...
	elseif talk[cid] == 1 then
		if msgcontains(msg, "foresight of the authorities") then
