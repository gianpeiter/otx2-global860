...
local tokens = {
	...
	[4] = {
		"Ye' brought the thirty tokens needed to advance to the fifth vampire hunter rank. You're cpletely obliterative, kid! Let me share some of my experience with ye'.",
		30,
		50000,
	},
	[5] = {
...
function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "token") then
		...
		npcHandler:say(
			cpl and "You are the highest vampire hunter rank now." or "Would ye' like to give me vampire tokens?",
			cid
		)
	...
	elseif msgcontains(msg, "vampire") and msgcontains(msg, "hunter") then
		npcHandler:say(
			"Yeah, that's my profession. If ye' want to learn more about it, ye will have to get some field experience. Nothing I could tell ye' would prepare ye' for the real thing.",
			cid
		)
		Topic[cid] = 0
	elseif msgcontains(msg, "inquisition") or msgcontains(msg, "mission") then
		...
		elseif v == 1 then
			npcHandler:say(
				"So they've sent another one? I just hope ye' better than the last one. Are ye' ready for a mission?",
				cid
			)
			Topic[cid] = 2
		elseif v >= 2 and v <= 21 then
			npcHandler:say(
				"So far ye've brought me "
					...
					.. " of 20 vampire dusts. Do ye' have any more with ye'?",
				cid
			)
			Topic[cid] = 3
		elseif v == 22 or v == 23 then
			...
			npcHandler:say(
				"Before ye' open his coffin and drag that beast out to destroy it, I advise ye' to place some garlic necklaces on the stone slabs next to his coffin. That will weaken himconsiderably. ...",
				cid,
				11000
			)
	...
	elseif Topic[cid] == 2 and msgcontains(msg, "yes") then
		npcHandler:say(
			"As they might have told ye', I'm a vampire hunter. If ye' want to be of any help, ye' can assist me with some of my tasks and bring me 20 ounces of vampire dust. This gives me some time to concentrate on a bigger project.",
			cid
		)
