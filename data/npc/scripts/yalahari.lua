...
function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "mission") then
		...
		elseif v == 2 then
			...
			npcHandler:say(
				"The worst plague, however, are unknown diseases that have spread in this quarter and eradicated any human population. We must stop it before other quarters are afflicted. We already identified certain carriers responsible for spreading the plague. ...",
				cid,
				5000
			)
			npcHandler:say(
				"It will be your task to eliminate them. This spell will protect you from becoming infected yourself. Enter the alchemist quarter and kill the three plague carriers, and at best anything else you might find there. ...",
				cid,
				11000
			)
			npcHandler:say(
		...
		elseif v == 9 and getPlayerStorageValue(cid, 100048) == -1 then
			if getPlayerStorageValue(cid, 100040) == 1 then
				...
				npcHandler:say(
					"There is no charm with you! You better don't return here until you've returned the charm.",
					cid
				)
			end
		elseif v == 9 then
		...
		elseif v == 11 and getPlayerStorageValue(cid, 100022) == 1 then
			if
				getPlayerStorageValue(cid, 100028) == 1
				and getPlayerStorageValue(cid, 100029) == 1
				and getPlayerStorageValue(cid, 100030) == 1
			then
				npcHandler:say(
		...
		elseif v == 12 then
			if getPlayerStorageValue(cid, 100024) == 1 then
				npcHandler:say(
					"Impressive! Still, I see your potential. So I ask you to rethink your loyalties. Come back if you have decided to which side you want to belong.",
					cid
				)
		...
		elseif v == 13 then
			npcHandler:say(
				"I'm quite aware of your connection to this manipulative Palimuth. Still, I see your potential. So I ask you to rethink your loyalties. Come back if you have decided to which side you want to belong.",
				cid
			)
			setPlayerStorageValue(cid, storage, 14)
		elseif v == 14 then
		...
		elseif v == 15 then
			...
			npcHandler:say(
				"Only the most prominent Yalahari are allowed to join the festivities. I assume you can imagine what honour it is that you've been invited to join us. Meet us in the inner city's centre. ...",
				cid,
				4000
			)
			npcHandler:say(
		...
		elseif v == 16 then
			if getPlayerStorageValue(cid, 100051) == 1 then
				npcHandler:say(
					"You are now allowed to enter the reward room. Also take this outfit as a reward to.",
					cid
				)
	...
	elseif Topic[cid] == 3 and getPlayerStorageValue(cid, 100032) == -1 then -- Finish mission 1 - The interrogation
		if msgcontains(msg, "yes") then
			if
				getPlayerStorageValue(cid, 100025) == 1
				and getPlayerStorageValue(cid, 100026) == 1
				and getPlayerStorageValue(cid, 100027) == 1
				and getPlayerStorageValue(cid, 100032) == -1
				and doPlayerTakeItem(cid, 9733, 1)
			then
				npcHandler:say(
