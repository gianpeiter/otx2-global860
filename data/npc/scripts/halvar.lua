...
function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'fight') or msgcontains(msg, 'pit') or msgcontains(msg, 'challenge') or msgcontains(msg, 'arena') then
		...
		if arenaId < 1 then
			arenaId = 1
			setPlayerStorageValue(cid, 51710, arenaId)
		end
		...
		if ARENA[arenaId] then
			npcHandler:say(
				...
				'} is ' ..
				ARENA[arenaId].price ..
				' gold pieces. Do you really want to participate and pay the {fee}?',
				cid
			)
			talkState[cid] = 1
		else
	...
	elseif talkState[cid] == 1 then
		...
		if msgcontains(msg, 'yes') then
			...
			if doPlayerRemoveMoney(cid, ARENA[arenaId].price) then
				...
				npcHandler:say(
					'As you wish! You can pass the door now and enter the teleporter to the pits.',
					cid
				)
...
keywordHandler:addKeyword({'greenhorn'}, StdModule.say, {npcHandler = npcHandler, text = 'That is the easiest way in our arena. The {fee} is 1000 gold. We were setting this up for our children to challenge some easy monsters and train them for the future.'})
