function onThink(interval)
	if tonumber(os.date("%d")) ~= getGlobalStorageValue(23456) then
		...
		db.executeQuery("UPDATE `players` SET `exphist7`=`exphist6`, `exphist6`=`exphist5`, `exphist5`=`exphist4`, `exphist4`=`exphist3`, `exphist3`=`exphist2`, `exphist2`=`exphist1`, `exphist1`=CAST(`experience` AS SIGNED) - CAST(`exphist_lastexp` AS SIGNED), `exphist_lastexp`=`experience`;")
	end
